...
import "../../admin/userList/UserList.css";
...
const MovieInterested = () => {
  ...
  const requestSearch = (searched) => {
    setCurrentPage(0);
    let filteredData = interested?.filter(
      (data) =>
        `${data?.userDetails?.firstName?.toLowerCase()} ${data?.userDetails?.lastName?.toLowerCase()}`?.includes(
  ...
  const getInterestedList = () => {
    PagesIndex.DataService.get(
      PagesIndex.Api.INTERESTED_UNINTERESTED_MOVIES_LIST
    )
      .then((res) => {
  ...
  const generateExcel = async () => {
    const headers = ["User Details", "Movie Name", "Date"];
    const rows = filteredData.map((item) => ({
      userDetails: `${item?.userDetails?.email || ''} ${item?.userDetails?.mobileNumber || ''}`,
      movieName: item?.movieDetails?.name ? item?.movieDetails?.name : "-",
    ...
    PagesIndex.XLSX.writeFile(
      ...
      )}_Contact_us_list.xlsx`,
      { compression: true }
    );
  };
  if (
    adminLoginData?.type == "Admin" ||
    adminLoginData?.roleId?.permissions?.includes("movie_interested_view")
  ) {
    return (
      <>
        <Index.Box className="">
            ...
            <Index.Box className="page-table-main">
              ...
              >
                ...
                >
                  <Index.TableHead>
                    <Index.TableRow>
                      <Index.TableCell colSpan={2}>User Details</Index.TableCell>
                                ...
                                <img
                                  src={
                                    item?.userDetails?.profile
                                      ? `${PagesIndex?.IMAGES_API_ENDPOINT}/${item?.userDetails?.profile}`
