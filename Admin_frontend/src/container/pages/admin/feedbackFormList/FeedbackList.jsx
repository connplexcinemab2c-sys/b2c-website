...
const FeedbackList = () => {
  ...
  const requestSearch = (searched) => {
    setCurrentPage(0);
    let filteredData = feedbackList?.filter(
      (data) =>
        (data?.name &&
  ...
  const getFeedbacks = () => {
    PagesIndex.DataService.get(PagesIndex.Api.GET_FEEDBACK_LIST)
      .then((res) => {
  ...
  const generateExcel = async () => {
    const headers = ["User", "Email", "Phone Number", "City", "Cinema Name", "Message", "Date"];
    const rows = filteredData?.map((item) => ({
      name: item?.name ? item?.name : "-",
      email: item?.email ? item?.email : "-",
      mobileNumber: item?.mobileNumber ? item?.mobileNumber : "-",
      city: item?.city ? item?.city : "-",
      cinemaName: item?.cinemaId?.cinemaName ? item?.cinemaId?.cinemaName : "-",
    ...
    PagesIndex.XLSX.writeFile(
      workbook,
      `Feedbacks_list_${PagesIndex.moment().format(
  ...
  if (
    ...
    adminLoginData?.roleId?.permissions?.includes("feedback_view")
  ) {
    return (
      <>
        <Index.Box className="">
                        ...
                        filteredData
                          ...
                          ?.map((item, index) => (
                            ...
                            >
                              ...
                              <Index.TableCell>
                                {item?.cinemaId?.cinemaName ? item?.cinemaId?.cinemaName : "-"}
                              </Index.TableCell>
                              <Index.TableCell>
                                {item?.createdAt
        ...
        >
          ...
          >
            ...
            <Index.Box className="modal-body">
              <Index.Box className="feedback-vide-details">{message}</Index.Box>
